task("rtl", function()
  set_menu {
    ...
    description = "Generate RTL (default Subsys top)",
    options = {
...
task("rtl_bridge_cfg", function()
  set_menu {
    ...
    description = "Generate AxiBridgeCfg (CFG bridge 1S x 3M) RTL",
    options = {
        {'b', "build-dir", "kv", "build", "build directory"},
    }
  }
  on_run(function()
    import("core.base.option")
    local build_dir = option.get("build-dir") or "build"
    local rtl_dir = path.join(build_dir, "rtl_bridge_cfg")

    local chisel_opts = {"-i", "test.runMain", "generator.AxiBridgeCfgGenerator"}
    table.join2(chisel_opts, {"--throw-on-first-error", "--target", "systemverilog", "--split-verilog", "--full-stacktrace", "-td", rtl_dir})
